...
export default function ComparisonBar() {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 max-w-full">
      <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-gray-200 dark:border-zinc-700 p-4 flex items-center justify-between gap-4 w-full relative">
        {/* Minimize button */}
        <button
        ...
        <div className="grid grid-flow-col items-center gap-2 overflow-x-auto scrollbar-hide">
          {cars.map((car) => {
            const thumb =
            ...
            return (
              <div
                key={car._id}
                className="relative group flex min-w-fit w-[240px] rounded-lg p-1 gap-2 border border-gray-200 dark:border-zinc-700 text-zinc-800 dark:text-zinc-200"
              >
                <img
                  src={thumb}
                  alt={car.name}
                  className="w-16 h-16 object-cover rounded-lg basis-1/3"
                />
                <div className="flex flex-col basis-2/3 p-2">
                  <p className="text-sm font-medium">{car.name}</p>
                  <p className="text-xs text-gray-500">{car.currentPrice}</p>
                </div>
                <button
                  onClick={() => removeCar(car._id)}
                  className="absolute top-1/2 -translate-y-1/2 right-4 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                >
                  ×
                </button>
              </div>
